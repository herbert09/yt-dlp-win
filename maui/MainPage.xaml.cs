...
public partial class MainPage : ContentPage
{
    ...
    private async void btnTaskActions_Clicked(object sender, EventArgs e)
    {
        if (sender is Button btn && btn.CommandParameter is DownloadTask task)
        {
            _selectedTask = task;
            var actions = new List<string> { "复制链接", "打开所在目录", "删除任务" };
            if (task.Status == "下载中") actions.Insert(0, "暂停");
            if (task.Status == "已暂停") actions.Insert(0, "继续");

            var action = await DisplayActionSheet("操作", "取消", null, actions.ToArray());
            switch (action)
            {
                case "暂停": PauseTask(task); break;
                case "继续": await ResumeTask(task); break;
                case "复制链接": await CopyTaskUrl(task); break;
                case "打开所在目录": OpenTaskDir(task); break;
                case "删除任务": DeleteTask(task); break;
            }
        }
    }
    ...
    private void DownloadVideo(DownloadTask task)
    {
        ...
        MainThread.BeginInvokeOnMainThread(() => task.Status = "获取信息中");
        ...
        MainThread.BeginInvokeOnMainThread(() => task.Title = string.IsNullOrEmpty(uploader) ? title : $"[{uploader}] {title}");
        ...
        MainThread.BeginInvokeOnMainThread(() => task.Status = "下载中");
        ...
        void HandleLine(string? rawLine)
        {
            ...
            if (match.Success && double.TryParse(match.Groups[1].Value, out var pct))
            {
                ...
                if (task.Progress != newProgress)
                    MainThread.BeginInvokeOnMainThread(() => task.Progress = newProgress);
            }
        ...
        if (process.ExitCode != 0)
        {
            if (task.IsCancelled)
            {
                MainThread.BeginInvokeOnMainThread(() => task.Status = "已暂停");
                return;
            }
    ...
    private async Task ResumeTask(DownloadTask task)
    {
        task.Status = "等待中";
        task.Progress = 0;
    ...
    private async Task CopyTaskUrl(DownloadTask task)
    {
